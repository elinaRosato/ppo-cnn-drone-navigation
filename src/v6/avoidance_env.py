"""
GPS-Free Obstacle Avoidance Environment — v6
...
Key differences from v5:
  ...
  - Stronger fog randomisation 0–12% (was 0–8%)
...
Observation (actor): Dict {
    ...
    "state":      (1,) float32  — accumulated smoothed action (unbounded centering proxy)
    "privileged": (PRIVILEGED_DIM,) float32  — critic only (not seen by actor at deploy)
}

Action: (1,) float32 in [−1, 1]
    Lateral correction; fwd = fixed_speed − max_lateral × |action|  (min fwd = 0.2 m/s)
"""
...
class DroneAvoidanceEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.client.moveToZAsync(self._episode_altitude, 2.0).join()
        ...
        return {
            ...
            "state":      np.array([0.0], dtype=np.float32),  # always 0 at episode start
    ...
    def step(self, action):
        ...
        fwd_speed  = self.fixed_speed - self.max_lateral * action_abs   # min vx = fixed_speed - max_lateral = 0.2
        ...
        return (
            {
                "image":      self.frame_stack.copy(),
                "state":      np.array([self._accumulated_action], dtype=np.float32),
                "privileged": self._privileged_vector.copy(),
            },
            float(reward),
            terminated,
