...
class DepthEstimator:
    ...
    def __init__(
        ...
        self._timing_log_interval = 100   # print timing summary every N calls
    ...
    def estimate(self, rgb: np.ndarray, training: bool = False,
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", enabled=use_fp16):
            outputs = self._model(**inputs)
            raw = outputs.predicted_depth.float()  # (1, H, W) — cast back to fp32
        _inference_ms = (time.perf_counter() - _t0) * 1000

        self._timing_calls    += 1
        self._timing_total_ms += _inference_ms
        if self._timing_calls % self._timing_log_interval == 0:
            avg_ms = self._timing_total_ms / self._timing_calls
            print(f"[DepthEstimator] inference: {_inference_ms:.1f} ms (avg over {self._timing_calls} calls: {avg_ms:.1f} ms)")
