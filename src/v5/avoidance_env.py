"""
GPS-Free Obstacle Avoidance Environment — v5

Key differences from v4:
  - Actor observation now includes accumulated smoothed action as a centering signal.
    This gives the actor a GPS-free proxy for lateral displacement from the baseline:
    positive = drifted right, negative = drifted left, ~0 = near center.
    Normalised by max_steps → stays in [-1, 1].
  - Lateral penalty kept: actor can now observe the quantity it is penalised for.

Observation (actor): Dict {
    "image":      (STACK_FRAMES, H, W) float32  — stacked estimated depth frames [0,1]
    "state":      (1,) float32  — accumulated smoothed action (unbounded)
