"""
Train GPS-free obstacle avoidance — v7.

Actor input : 3 strided estimated depth frames (3,144,192) float32 [0,1]  [4:3 IMX219, stride-4]
            + state (2,): [accumulated_action, prev_smoothed_action]
...
Architecture:
  Actor  : CNN → 256 features → cat(state 2) → 258 → MLP[128,128] → action
...
ACTOR_FEATURES_DIM = CNN_FEATURES_DIM + STATE_DIM   # 258 — total actor features
...
class ActorFeaturesExtractor(BaseFeaturesExtractor):
    """
    Extracts features from 'image' (3-frame strided depth stack) and 'state'
    ([accumulated_action, prev_smoothed_action]).

    Pipeline: image → CNN → 256-dim → cat(state 2-dim) → 258-dim output
    ...
    def forward(self, obs):
        ...
        state        = obs["state"]                             # (B, 2)
        return torch.cat([cnn_features, state], dim=1)          # (B, 258)
...
class AvoidanceCallback(BaseCallback):
    ...
    Logs per rollout:
      ...
      reward/forward, reward/action_smooth, reward/consistency, reward/depth_penalty, reward/lateral_penalty
    ...
    def _on_step(self) -> bool:
        ...
        for done, info in zip(dones, infos):
            ...
            if 'ep_consistency_reward' in info:
                self._rollout_consist_rew.append(info['ep_consistency_reward'])
    ...
    def _on_rollout_end(self) -> None:
        ...
        _log_if("reward/consistency",           self._rollout_consist_rew)
        ...
        self._rollout_consist_rew.clear()
        ...
        if DroneAvoidanceEnv._batch_complete:
            ...
            if win_rate >= self.best_win_rate + 0.05:
                ...
                path  = os.path.join(
                    self.save_dir,
                    f"best_avoidance_v7_{ts}_win{int(win_rate * 100)}_s{stage}"
                )
...
def train(resume=False, checkpoint=None, target_steps=None, use_ros2=False, start_stage=0, speed_scale=1):
    _here          = os.path.dirname(os.path.abspath(__file__))
    base_model_dir = os.path.join(_here, "../../models_v7")
    base_log_dir   = os.path.join(_here, "../../logs_v7")
    ...
    print("=" * 70)
    print("GPS-FREE OBSTACLE AVOIDANCE — v7")
    print("=" * 70)
    ...
    if speed_scale > 1:
        print(f"Fast mode:    {speed_scale}× speed  |  {step_hz} Hz  |  deploy at 1× / 10 Hz")
    print("=" * 70)
        ...
        if run_dir:
            ...
            if os.path.exists(ckpt_dir):
                zips = [f for f in os.listdir(ckpt_dir)
                        if f.endswith('.zip') and f.startswith('avoidance_v7_')]
                if zips:
    ...
    if not resume and not checkpoint:
        ...
        model = PPO(
            ...
            policy_kwargs=dict(
                ...
                features_extractor_kwargs=dict(features_dim=ACTOR_FEATURES_DIM),  # 258
    ...
    callbacks = [
        CheckpointCallback(
            save_freq=30_000, save_path=ckpt_dir, name_prefix="avoidance_v7"
        ),
    ...
    try:
        model.learn(
            ...
            tb_log_name="avoidance_v7",
        )
        final_path = os.path.join(run_dir, "avoidance_v7_final")
        ...
        model.save(os.path.join(run_dir, "avoidance_v7_interrupted"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train GPS-free avoidance — v7")
    ...
    parser.add_argument("--fast",       type=int, default=1, choices=[1, 2, 4],
                        help="Speed multiplier: 2=2× speed/20Hz, 4=4× speed/40Hz (deploy at 1×/10Hz)")
