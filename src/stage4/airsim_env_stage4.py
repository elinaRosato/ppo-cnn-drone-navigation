"""
Stage 4 Environment: Full Navigation with Obstacles
Adds obstacle avoidance using Blocks environment
Camera-based vision for detecting obstacles
"""
...
class AirSimStage4Env(gym.Env):
    """Stage 4: Navigate to goals while avoiding obstacles"""

    metadata = {'render_modes': ['rgb_array']}

    def __init__(self,
                 goal_range_x=(10, 30),  # Further goals with obstacles
                 goal_range_y=(10, 30),
                 goal_range_z=(-5, -1),
                 goal_radius=3.0,
                 min_altitude=-10.0,
                 max_altitude=0.0):
        super(AirSimStage4Env, self).__init__()
        ...
        self.img_height = 84
        self.img_width = 84
        self.max_steps = 1000  # Longer episodes for obstacle navigation

        self.client = airsim.MultirotorClient()
        self.client.confirmConnection()
        print("Connected to AirSim!")
        print("⚠️  Make sure you're using the 'Blocks' environment in AirSim!")
        ...
        # Observation space: camera + full state + goal + altitude info
    ...
    def reset(self, seed=None, options=None):
        ...
        print(f"\n[STAGE 4] Goal: ({goal_x:.1f}, {goal_y:.1f}, {goal_z:.1f}) | Distance: {distance:.1f}m")
        print(f"[STAGE 4] Obstacles present - use camera to navigate!")
    ...
    def step(self, action):
        ...
        if extreme_altitude:
            print(f"[ALTITUDE VIOLATION] Alt: {position[2]:.2f}m")
            # Auto-correct
        ...
        if goal_reached:
            print(f"[SUCCESS] Goal reached! Distance: {distance:.2f}m | Steps: {self.current_step} | Collisions: {self.collision_count}")

        # Calculate reward
        reward = self._calculate_reward(position, distance, goal_reached, collision, extreme_altitude)
        ...
        if terminated or truncated:
            print(f"[STAGE 4] Episode ended | Reward: {self.episode_reward:.2f} | Steps: {self.current_step} | Collisions: {self.collision_count}")
            if truncated:
    ...
    def _calculate_reward(self, position, distance, goal_reached, collision, extreme_altitude):
        """
        Stage 4 Reward: Goal progress + Obstacle avoidance + Altitude constraints
        """
        reward = 0.0

        # Progress toward goal (still dominant)
        ...
        # Collision penalty (non-terminating)
        if collision:
            reward -= 50.0
        ...
        # Soft altitude boundary penalty
        altitude_margin = min(abs(position[2] - self.max_altitude), abs(position[2] - self.min_altitude))
        if altitude_margin < 1.0:
            reward -= (1.0 - altitude_margin) * 5.0

        # Goal bonus
        if goal_reached:
            # Bonus for reaching goal + bonus for fewer collisions
            reward += 1000.0
            collision_bonus = max(0, 500.0 - (self.collision_count * 50.0))
