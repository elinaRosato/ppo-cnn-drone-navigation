"""
Train GPS-free obstacle avoidance — v4.

Actor input : 3 stacked estimated depth frames (3,192,192) float32 [0,1]
              NO state vector (fully GPS-free at deploy time)
...
Architecture:
  Actor  : CNN → 256 features → MLP[64,64] → action
...
ACTOR_FEATURES_DIM = 256   # CNN head output only — no state MLP


# ── Actor feature extractor (CNN, ignores privileged) ─────────────────────────

class ActorFeaturesExtractor(BaseFeaturesExtractor):
    """
    Extracts features from 'image' only (3-frame estimated depth stack).
    ...
    def __init__(self, observation_space: spaces.Dict, features_dim: int = ACTOR_FEATURES_DIM):
        ...
        assert features_dim == ACTOR_FEATURES_DIM, \
            f"features_dim must be {ACTOR_FEATURES_DIM}"
        ...
        self.cnn_head = nn.Sequential(nn.Linear(cnn_out, ACTOR_FEATURES_DIM), nn.ReLU())

    def forward(self, obs):
        return self.cnn_head(self.cnn(obs["image"]))
    ...
    def _on_rollout_end(self) -> None:
        ...
        if DroneAvoidanceEnv._batch_complete:
            ...
            if win_rate >= self.best_win_rate + 0.05:
                ...
                path  = os.path.join(
                    self.save_dir,
                    f"best_avoidance_v4_{ts}_win{int(win_rate * 100)}_s{stage}"
                )
...
def train(resume=False, target_steps=None, use_ros2=False, start_stage=0):
    _here          = os.path.dirname(os.path.abspath(__file__))
    base_model_dir = os.path.join(_here, "../../models_v4")
    base_log_dir   = os.path.join(_here, "../../logs_v4")
    os.makedirs(base_model_dir, exist_ok=True)
    os.makedirs(base_log_dir,   exist_ok=True)

    print("=" * 70)
    print("GPS-FREE OBSTACLE AVOIDANCE — v4")
    print("=" * 70)
    print("\nActor input:  3× estimated depth frames (3,144,256) float32 [0,1]  [16:9]")
    ...
    depth_estimator = DepthEstimator(target_size=(256, 144), device=device)

    # Optional ROS2 bridge
    ros2_bridge = None
    if use_ros2:
        from ros2_bridge import ROS2CameraBridge
        ros2_bridge = ROS2CameraBridge(target_size=(256, 144))
        ros2_bridge.start()
    ...
    callbacks = [
        CheckpointCallback(
            save_freq=30_000, save_path=ckpt_dir, name_prefix="avoidance_v4"
        ),
    ...
    try:
        model.learn(
            ...
            tb_log_name="avoidance_v4",
        )
        final_path = os.path.join(run_dir, "avoidance_v4_final")
        ...
        model.save(os.path.join(run_dir, "avoidance_v4_interrupted"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train GPS-free avoidance — v4")
