"""
GPS-Free Obstacle Avoidance Environment — v7

Key differences from v6:
  - IMX219 camera: 192×144 (4:3 native aspect ratio), FOV 62°
  - Per-episode domain randomisation for sim-to-real transfer:
      * Cruising altitude jitter ±0.3 m
      * Ambient lateral wind drift U(−0.3, 0.3) m/s
  - Stronger fog randomisation 0–12%
  - action_magnitude passed to depth estimator for motion-blur augmentation
  - CLAHE + occlusion blobs added in depth_estimator.py
  - Strided depth frame stack: 3 frames at stride FRAME_STRIDE (0.8 s lookback at 10 Hz)
  - State: [accumulated_action, prev_smoothed_action] (2-dim)
  - Forward speed soft-decoupled: fwd = fixed_speed * max(MIN_FWD_FACTOR, 1 - |action|)
    lateral is fully independent

Observation (actor): Dict {
    "image":      (3, H, W) float32  — strided depth frames [0,1], stride FRAME_STRIDE steps
    "state":      (2,) float32  — [accumulated_action, prev_smoothed_action]
    "privileged": (PRIVILEGED_DIM,) float32  — critic only (not seen by actor at deploy)
}

Action: (1,) float32 in [−1, 1]
    Lateral correction; fwd = fixed_speed * max(MIN_FWD_FACTOR, 1 - |action|)
    Lateral speed is independent: lat = max_lateral * action
"""
...
CONSISTENCY_WEIGHT   = 0.1    # penalty weight for large action changes between steps
MIN_FWD_FACTOR       = 0.6    # minimum forward speed fraction even at max lateral action
FRAME_STRIDE         = 4      # depth frame temporal stride — 0.8 s lookback at 10 Hz
...
class DroneAvoidanceEnv(gym.Env):
    ...
    _stage_episode_count      = 0   # episodes completed at current stage
    MIN_EPISODES_PER_STAGE    = 300  # must see ≥300 episodes before advancing
    ...
    def __init__(
        ...
        self.fixed_speed       = fixed_speed * speed_scale
        self.max_lateral       = max_lateral * speed_scale
        self._step_duration    = 1.0 / step_hz
        ...
        self._ep_consistency_reward = 0.0
        self._action_history        = []   # smoothed lateral actions for stats
        self._accumulated_action    = 0.0  # running sum of smoothed actions — centering proxy
        self._prev_smoothed_action  = 0.0  # previous step's smoothed action — consistency signal
        ...
        # Step timing accumulators (reset every _TIMING_LOG_INTERVAL steps)
        self._TIMING_LOG_INTERVAL = 200
        self._timing_step_count   = 0
        self._timing_step_total   = 0.0
        self._timing_depth_total  = 0.0

        # Action smoothing (1-step fixed pipeline delay)
        self._delayed_action = np.zeros(1, dtype=np.float32)

        # Per-episode domain randomisation state
        ...
        n_bg = max(1, round(0.28 * len(self.tree_names)))  # ~20 bg out of 70 total
        ...
        self.observation_space = spaces.Dict({
            ...
            "state": spaces.Box(
                ...
                shape=(2,),   # [accumulated_action, prev_smoothed_action]
                dtype=np.float32,
            ),
    ...
    def _spawn_corridor_trees(self):
        ...
        # Build a base ellipse along X+, centred ahead of spawn.
        # ref_dist matches CORRIDOR_LENGTH so the ellipse covers the actual
        # episode distance rather than an oversized 100 m reference.
        ref_dist = CORRIDOR_LENGTH         # 30 m — matches episode length
    ...
    def _place_background_trees(self, obstacle_positions):
        ...
        sx, sy  = float(self.spawn_pos[0]), float(self.spawn_pos[1])
        MAX_R   = 60.0    # 2× corridor length — trees further than this are never in camera view
        MIN_GAP = 5.0
    ...
    def _run_depth_estimation(self, action_magnitude: float = 0.0):
        """Estimate depth from latest RGB, push into the frame buffer, and build strided stack."""
        if self.depth_estimator is not None:
            est = self.depth_estimator.estimate(
                self._latest_rgb,
        ...
        self._frame_buffer.append(est)

        # Sample at strides: frame_stack[0]=oldest (t-8), frame_stack[2]=newest (t)
    ...
    def reset(self, seed=None, options=None):
        ...
        self._prev_smoothed_action  = 0.0

        # Per-episode domain randomisation
        self._episode_altitude = self.cruising_altitude + random.uniform(-0.3, 0.3)  # altitude jitter

        self._episode_stage = DroneAvoidanceEnv._current_curriculum_stage

        # Hard reset clears collision state and physics
        self.client.reset()
        time.sleep(0.1)  # AirSim needs a brief settle after reset before API calls
        self.client.enableApiControl(True)
        self.client.armDisarm(True)

        # Takeoff engages the flight controller (spins rotors to hover mode).
        # Without this, simSetVehiclePose teleports the body but gravity pulls it down.
        self.client.takeoffAsync().join()

        # Teleport to exact episode altitude + yaw=0 (facing X+).
        # Replaces the slow moveToZAsync + rotateToYawAsync sequence (~4-5s saved).
        self.client.simSetVehiclePose(
            airsim.Pose(
                airsim.Vector3r(0.0, 0.0, self._episode_altitude),
                airsim.Quaternionr(0, 0, 0, 1),  # yaw=0 → facing X+
            ),
            ignore_collision=True,
        )
        # Re-engage hover at teleported position so the flight controller tracks it
        self.client.moveByVelocityAsync(0, 0, 0, 1.0)
        time.sleep(0.1)  # brief settle

        # Spawn pos is always world origin — set directly, no need to query AirSim
        self.spawn_pos = np.array([0.0, 0.0, self._episode_altitude], dtype=np.float32)
        self._pos      = self.spawn_pos.copy()

        # Domain randomisation — done after drone is positioned so the environment is
        # fully configured when the camera captures its first frame.
        # AirSim treats datetime as UTC; default world is Redmond WA (UTC-7 summer).
        # 17-22 UTC → 10:00-15:59 local → guaranteed midday light, no horizon issues.
        ...
        self.client.simSetWeatherParameter(
            airsim.WeatherParameter.Fog, random.uniform(0.0, 0.12)
        )

        # Tree placement uses spawn_pos which is now set
        ...
        # Initialise frame buffer with the first depth estimate repeated across all slots
        # so every strided index starts with a real (not zero) frame.
        self._fetch_rgb_and_depth()
        self._update_state()
        self._prev_x = float(self._pos[0])

        if self.depth_estimator is not None:
            init_frame = self.depth_estimator.estimate(
                self._latest_rgb, training=self.training_mode, action_magnitude=0.0
            )
        else:
        ...
        self.frame_stack = np.stack([init_frame] * self.stack_frames, axis=0)
        ...
        print(
            ...
            f"alt={self._episode_altitude:.2f}m"
        )
    ...
    def step(self, action):
        ...
        fwd_speed  = self.fixed_speed * max(MIN_FWD_FACTOR, 1.0 - action_abs)  # min vx = MIN_FWD_FACTOR * fixed_speed
        lat_speed  = self.max_lateral * float(smoothed[0])                      # fully independent of fwd
        alt_error  = float(self._episode_altitude) - float(self._pos[2])
        ...
        self._run_depth_estimation(action_magnitude=action_abs)
        _depth_ms = (time.perf_counter() - _t_depth) * 1000
        self._update_state()
        ...
        # Penalise large changes in action between steps — teaches gradual corrections.
        # Actor observes prev_smoothed_action in state so it can anticipate this cost.
        reward_consistency = -CONSISTENCY_WEIGHT * abs(float(smoothed[0]) - self._prev_smoothed_action)
        ...
        self._ep_consistency_reward  += reward_consistency
        self._ep_depth_penalty       += dp
        self._ep_lateral_penalty     += lp
        self._action_history.append(float(smoothed[0]))
        self._accumulated_action    += float(smoothed[0])
        self._prev_smoothed_action   = float(smoothed[0])

        reward = reward_fwd + reward_action + reward_consistency + dp + lp
        if terminated:
        ...
        if terminated or truncated:
            ...
            print(
                ...
                f"consist={self._ep_consistency_reward:+.1f}  "
            ...
            DroneAvoidanceEnv._stage_episode_count += 1
            ...
            if DroneAvoidanceEnv._batch_episodes >= 50:
                ...
                eligible = DroneAvoidanceEnv._stage_episode_count >= DroneAvoidanceEnv.MIN_EPISODES_PER_STAGE
                advance = cur < 2 and win_rate >= 0.80 and eligible
                print(
                    f"\n{'='*60}\n"
                    f"[BATCH COMPLETE] 50 episodes | "
                    f"win_rate={win_rate:.0%} | "
                    f"Stage {cur + 1} ({stage_names[cur]})"
                    + (f" → advancing to Stage {cur + 2}!" if advance
                       else f" | no advance (eps at stage: {DroneAvoidanceEnv._stage_episode_count}/{DroneAvoidanceEnv.MIN_EPISODES_PER_STAGE})")
                    + f"\n{'='*60}\n",
                    flush=True,
                )
                ...
                if advance:
                    ...
                    DroneAvoidanceEnv._stage_episode_count = 0

        # Rate-limit to target Hz — sleep the remaining time in this step
        _elapsed = time.perf_counter() - _t_step_start
        _remaining = self._step_duration - _elapsed
        if _remaining > 0:
            time.sleep(_remaining)
        _total_ms = (time.perf_counter() - _t_step_start) * 1000
        self._step_times.append(_total_ms / 1000)

        # Step timing log — print averages every N steps
        self._timing_step_count  += 1
        self._timing_step_total  += _elapsed * 1000   # work time (before sleep)
        self._timing_depth_total += _depth_ms
        if self._timing_step_count % self._TIMING_LOG_INTERVAL == 0:
            avg_work  = self._timing_step_total  / self._timing_step_count
            avg_depth = self._timing_depth_total / self._timing_step_count
            budget_ms = self._step_duration * 1000
            hz        = 1.0 / self._step_duration
            print(
                f"[TIMING] step work: {avg_work:.1f} ms  |  "
                f"depth: {avg_depth:.1f} ms  |  "
                f"other: {avg_work - avg_depth:.1f} ms  "
                f"(budget @ {hz:.0f} Hz: {budget_ms:.0f} ms)  "
                f"[avg over {self._timing_step_count} steps]"
            )
            self._timing_step_count  = 0
            self._timing_step_total  = 0.0
            self._timing_depth_total = 0.0
        ...
        info = {
            ...
            "ep_consistency_reward": self._ep_consistency_reward,
        ...
        return (
            {
                "image":      self.frame_stack.copy(),
                "state":      np.array([self._accumulated_action, self._prev_smoothed_action], dtype=np.float32),
                "privileged": self._privileged_vector.copy(),
            },
            float(reward),
            terminated,
