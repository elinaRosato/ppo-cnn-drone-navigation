"""
Live debug viewer — v4 (8-panel tiled display, 16:9).
...
Image resolution: 256×144 (16:9).  Tiles displayed at 320×180.

Depth Anything V2 outputs a single-channel float32 disparity map — not RGB.
The model receives exactly the grayscale tiles shown in cols 2-3, row 1.

GT AirSim depths (col 4) use COLORMAP_HOT: black=far, white=close.
These are training-only signals — the actor never sees them.
In v4 the critic uses tree positions (not depth), so the 15m panel is for
...
STACK_FRAMES   = 3
IMG_W          = 256    # must match avoidance_env.py / settings.json
IMG_H          = 144
TILE_W         = 320    # display tile size (16:9 at 1.25× source)
TILE_H         = 180
...
def main(fps: int, augment: bool, scene_topic: str, depth_topic: str):
    ...
    while True:
        ...
        est_depth = estimator.estimate(rgb_aug, training=False)
        ...
        tile_est      = gray_tile(est_depth, "depth est. (model input)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='v4 debug viewer — 8-panel 16:9 display')
