"""
Train GPS-free obstacle avoidance — v5.

Actor input : 3 stacked estimated depth frames (3,144,256) float32 [0,1]
...
Architecture:
  Actor  : CNN → 256 features → cat(state 1) → 257 → MLP[64,64] → action
    ...
    def _on_rollout_end(self) -> None:
        ...
        if DroneAvoidanceEnv._batch_complete:
            ...
            if win_rate >= self.best_win_rate + 0.05:
                ...
                path  = os.path.join(
                    self.save_dir,
                    f"best_avoidance_v5_{ts}_win{int(win_rate * 100)}_s{stage}"
                )
...
def train(resume=False, target_steps=None, use_ros2=False, start_stage=0):
    ...
    base_model_dir = os.path.join(_here, "../../models_v5")
    base_log_dir   = os.path.join(_here, "../../logs_v5")
    os.makedirs(base_model_dir, exist_ok=True)
    os.makedirs(base_log_dir,   exist_ok=True)

    print("=" * 70)
    print("GPS-FREE OBSTACLE AVOIDANCE — v5")
    print("=" * 70)
    ...
    callbacks = [
        CheckpointCallback(
            save_freq=30_000, save_path=ckpt_dir, name_prefix="avoidance_v5"
        ),
    ...
    try:
        model.learn(
            ...
            tb_log_name="avoidance_v5",
        )
        final_path = os.path.join(run_dir, "avoidance_v5_final")
        ...
        model.save(os.path.join(run_dir, "avoidance_v5_interrupted"))
