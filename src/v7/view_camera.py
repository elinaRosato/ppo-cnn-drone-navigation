"""
Live debug viewer — v7 (8-panel tiled display, 4:3 IMX219).

Shows everything in a single 4×2 grid window:

  Row 0 │ RGB (raw)  │ RGB (augmented) │ Depth est. + CLAHE │ AirSim 3.5m (penalty) │
  Row 1 │ Stack t-8  │ Stack t-4       │ Stack t-0 (latest) │ AirSim 15m  (debug)   │

Stack row shows the strided temporal stack the model receives (stride-4 = 0.8 s lookback).
...
STACK_FRAMES   = 3
FRAME_STRIDE   = 4   # must match avoidance_env.py
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='v7 debug viewer — 8-panel 4:3 display')
